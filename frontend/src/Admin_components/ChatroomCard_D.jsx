...
import { getUsername } from './adminLogin';

const ChatroomCard_D = ({ name, description, roomId,onDeleteSuccess }) => {
  ...
  const handleDelete=async(roomId)=>{
    const res=await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/delete?roomId=${roomId}`,
      {
        method:"DELETE"
      }
    );
      ...
      <div className="p-5 flex flex-col h-full">
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex-shrink-0 ml-4">
           {/* <div className="relative group w-10 h-10 rounded-full bg-gray-700 hover:bg-gray-900 flex items-center justify-center">
  
  <span className="absolute top-[-18px] text-[10px] text-white bg-gray-700 px-2 py-[2px] rounded opacity-0 group-hover:opacity-100 transition-opacity z-10">
    Delete
  </span>

  <button onClick={()=>handleDelete(roomId)}>
 ...
 <div className="relative group w-10 h-10 rounded-full bg-gray-700 hover:bg-gray-900 flex items-center justify-center">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-sm mx-4 text-center">
            <h2 className="text-lg font-semibold text-white mb-4">Are you sure you want to delete this room?</h2>
